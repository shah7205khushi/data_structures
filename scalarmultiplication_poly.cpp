...
// Function to multiply a polynomial with a scalar value
void multiplyPolynomial(Node* head, int scalar)
{
    while (head != NULL) {
        head->coeff *= scalar;
        head = head->next;
...
int main()
{
    ...
    cout << "Enter the scalar value to multiply with the polynomials: ";
    cin >> scalar;
    ...
    cout << "Multiplication with scalar value:" << endl;
    multiplyPolynomial(head1, scalar);
    printList(head1);
    cout << endl;
    multiplyPolynomial(head2, scalar);
    printList(head2);
